...
--
-- A rule matches when BOTH conditions pass:
--   category  absent / '' / 'ALL'  →  matches any item category
--             otherwise             →  item.category must equal rule.category
--   wildcard  absent / ''           →  matches any item name
--             otherwise             →  item.name must satisfy wildcard_match
function sorter.match_rule(item, rules)
    for _, rule in ipairs(rules) do
        -- Category condition.
        local cat_ok = (not rule.category or rule.category == '' or rule.category == 'ALL')
            or (item.category and item.category == rule.category)

        -- Wildcard condition.
        local name_ok = (not rule.wildcard or rule.wildcard == '')
            or wildcard_match(item.name, rule.wildcard)

        if cat_ok and name_ok then
            return rule
        end
    end
    return nil
                    ...
                    if tu.used >= tu.max then
                        ...
                        if hops == 2 and inv_usage.used >= inv_usage.max then
                            ...
                            plan.moves[#plan.moves + 1] = {
                                ...
                                count = item.count,
                                item_id = item.id,
                                id = item.id,               -- alias for the UI icon builder
                                ...
                                from_name = bag.name,
                                from_slot = item.slot,
                                to = target.key,            -- alias for mule highlight
                                to_id = target.id,
                                to_key = target.key,
                                to_name = target.name,
                                hops = hops,
