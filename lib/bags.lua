--[[
    ...
    Bag ids verified against the Windower Resources (bags.lua):
        0  inventory  (Gobbiebag)
        1  safe       (Mog Safe)
        2  storage    (Furniture Storage)
        3  temporary  (Temporary items — NOT sortable, excluded)
        4  locker     (Mog Locker)
        5  satchel    (Mog Satchel)
        6  sack       (Mog Sack)
        7  case       (Mog Case)
        8  wardrobe   (Mog Wardrobe)
        9  safe2      (Mog Safe 2)
        10 wardrobe2  (Mog Wardrobe 2)
    ...
    Note: bag id 3 (temporary items) is intentionally omitted because items
...
bags.list = {
    ...
    { id = 1,  key = 'safe',      name = 'Mog Safe',         max_slots = 80, note = 'Accessible in Mog House.' },
    { id = 2,  key = 'storage',   name = 'Furniture Storage', max_slots = 80, note = 'Uses placed furniture. Slot count depends on furniture.' },
    { id = 4,  key = 'locker',    name = 'Mog Locker',       max_slots = 80, note = 'Requires a paid lease (Imperial Bronze Pieces).' },
    ...
    { id = 8,  key = 'wardrobe',  name = 'Mog Wardrobe',     max_slots = 80, note = 'Equipment can be worn directly from a wardrobe.' },
    { id = 9,  key = 'safe2',     name = 'Mog Safe 2',       max_slots = 80, note = 'Requires Mog House 2nd floor upgrade.' },
    { id = 10, key = 'wardrobe2', name = 'Mog Wardrobe 2',   max_slots = 80, note = 'Equipment wardrobe.' },
